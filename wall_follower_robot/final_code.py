...
def run_robot(robot):
    ...
    # Increased to move approximately 2-3 tiles forward
        ...
        # Initialize variables
            ...
            if detected_color != "Unknown":
                if detected_color == "Red":
                    c_values[0] += 1
                    color_val = c_values[0]
                elif detected_color == "Yellow":
                    if c_values[0] > 0:
                        c_values[1] += 1
                        color_val = c_values[1]
                elif detected_color == "Pink":
                    if c_values[1] > 0: 
                        c_values[2] += 1
                        color_val = c_values[2]
                    ...
                    if not green_detected:
                        ...
                        if green_detected:
                            ...
                            while robot.getTime() - start_time < 3.98 :
                                # Keep the motors running forward
                                left_motor.setVelocity(max_speed)
                                right_motor.setVelocity(max_speed)
                                robot.step(timestep)  # Wait for the next time step
